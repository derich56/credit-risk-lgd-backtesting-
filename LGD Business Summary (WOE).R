...
############################################################
## 1. LOSS CONTRIBUTION BY CREDIT GRADE (PIE CHART)
############################################################
...
ggplot(seg_grade_plot,
  ...
  coord_polar(theta = "y") +
  labs(
    title = "Loss Contribution by Credit Grade",
    fill = "Credit Grade"
  ) +
  theme_minimal() +
  theme(
...
############################################################
## 2. LGD DRIVER IMPORTANCE
############################################################
driver_plot <- coef_table %>%
  mutate(
    value = abs(Coefficient),
    Feature = ifelse(Feature == "Others", "others", Feature)
  ) %>%
  arrange(desc(value)) %>%
  mutate(
    rank = row_number(),
    Feature = ifelse(rank <= 6, Feature, "others")
  ) %>%
  group_by(Feature) %>%
...
############################################################
## 3. LOSS DISTRIBUTION BY PURPOSE (TOP 6)
############################################################
...
ggplot(seg_purpose_plot,
  ...
  coord_polar("y") +
  labs(
    title = "Loss Distribution by Purpose (Top 6)",
    fill = "Purpose"
  ) +
  theme_minimal() +
  theme(
...
############################################################
## 4. LOSS DISTRIBUTION BY RISK SEGMENT
############################################################
...
ggplot(seg_risk_plot,
  ...
  coord_polar("y") +
  labs(
    title = "Loss Distribution by Risk Segment",
    fill = "Risk Segment"
  ) +
  theme_minimal() +
  theme(
...
############################################################
## 5. LGD TREND BY CREDIT GRADE (LINE CHART)
############################################################
...
ggplot(lgd_grade,
  ...
  labs(
    title = "LGD Trend by Credit Grade",
    x = "Credit Grade",
    y = "Average LGD"
  ) +
  theme_minimal()
...
############################################################
## 6. TOP 10 LOSS SEGMENTS (GRADE + PURPOSE)
############################################################
