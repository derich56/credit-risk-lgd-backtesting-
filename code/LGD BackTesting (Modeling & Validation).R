...
sig_vars <- intersect(sig_vars, colnames(train_df))

sig_vars <- sig_vars[!is.na(sig_vars)]

if(length(sig_vars) == 0){
  sig_vars <- colnames(train_df)
  sig_vars <- setdiff(sig_vars, "lgd")
}
...
qqnorm(res_lr, main="Normal Q-Q")
qqline(res_lr, col="red")
...
set.seed(123)
res_sample_lr <- sample(res_lr, min(5000,length(res_lr)))

shapiro_lr <- shapiro.test(res_sample_lr)
...
plot(fit_woe, res_woe,
     main="Residual vs Fitted",
     col="blue", pch=20)
abline(h=0,col="red")

qqnorm(res_woe, main="Normal Q-Q")
qqline(res_woe,col="blue")

hist(res_woe, main="Histogram", col="lightblue")
...
plot(pred_lgb, res_lgb,
     main="Residual vs Prediction",
     col="green", pch=20)
abline(h=0,col="red")

qqnorm(res_lgb, main="Normal Q-Q")
qqline(res_lgb,col="green")

hist(res_lgb, main="Histogram", col="lightgreen")

plot(pred_lgb, y_test,
     main="Actual vs Predicted",
     col="purple", pch=20)
abline(0,1,col="red", lwd=2)

set.seed(123)
res_sample_lgb <- sample(res_lgb, min(5000,length(res_lgb)))

shapiro_lgb <- shapiro.test(res_sample_lgb)
