...
make_trend_badge <- function(current, previous, higher_is_better = TRUE, is_percentage_metric = FALSE){
  ...
  if(is.null(previous) || length(previous) == 0 || is.na(previous) || previous == 0){
    return('<div class="trend-badge trend-neutral">• 
No prior data for comparison </div>')
  }
...
format_prev_period <- function(start_year, end_year){
  ...
  if(prev_start == prev_end){
    ...
    return(paste0(prev_start, "–", prev_end))
  }
}
  ...
  output$total_ead <- renderValueBox({
    ...
    valueBox(
      ...
      HTML(paste0(
        ...
        "<div style='font-size:12px;font-weight:500;opacity:0.85;margin-top:6px;'>",
        "Prev (", format_prev_period(input$year_range[1], input$year_range[2]), "): ",
        format_number(prev_val),
        "</div>"
